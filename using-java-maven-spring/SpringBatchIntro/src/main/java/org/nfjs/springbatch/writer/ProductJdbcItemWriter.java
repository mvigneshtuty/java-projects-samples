 ...
 * 
 */
package org.nfjs.springbatch.writer;
...
public class ProductJdbcItemWriter implements ItemWriter<Product> {
	...
	public void write(List<? extends Product> items) throws Exception {
		for (Product item : items) {
			System.out.println(item.getName() + ", " + item.getDesc() + ", " + item.getPrice() + ", " + item.getId());
			int updated = jdbcTemplate.update(UPDATE_PRODUCT, item.getName(), item.getDesc(), item.getPrice(),
					item.getId());
			if (updated == 0) {
				jdbcTemplate.update(INSERT_PRODUCT, item.getId(), item.getName(), item.getDesc(), item.getPrice());
			}
		}
	}
